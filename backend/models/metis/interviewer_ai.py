...
Start with a warm greeting and then begin asking questions one at a time."""
    ...
    def export_state(self) -> Dict:
        """
        Serialise interviewer state for database persistence.

        Returns a dict that can be stored in MongoDB and later passed
        to import_state() to resume the interview seamlessly.
        """
        return {
    ...
    def import_state(self, state: Dict) -> None:
        """
        Restore interviewer state from a previously exported snapshot.

        Args:
            state: Dict returned by a previous call to export_state()
        """
